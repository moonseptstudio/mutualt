...
const MatchRow = ({ match, currentUserId, onViewDetails, hasPackage, onUnlockRequire }: any) => {
    ...
    return (
        <div
            ...
            className={`group relative bg-(--bg-card) dark:bg-(--bg-card)/90 rounded-2xl sm:rounded-[32px] p-4 sm:p-6 border border-(--border-main) hover:border-${accentColor}-200/50 dark:hover:border-${accentColor}-800/50 shadow-sm transition-all duration-500 cursor-pointer overflow-hidden active:scale-[0.99]`}
        >
            {/* Background Accent */}
            ...
            <div className="relative flex flex-col xl:flex-row xl:items-center gap-8">
                ...
                <div className="flex items-center space-x-5 min-w-[280px]">
                    <div className="relative shrink-0">
                        <div className={`w-12 h-12 sm:w-16 sm:h-16 bg-slate-50 dark:bg-slate-800 rounded-xl sm:rounded-3xl overflow-hidden border-2 border-(--bg-main) shadow-sm relative z-10 transition-transform duration-500 group-hover:scale-110 ${otherPartner ? '-translate-x-1' : ''}`}>
                            <img src={getAvatarUrl(mainPartner.profileImageUrl, mainPartner.name)} alt="avatar" />

                        </div>
                        {otherPartner && (
                            <div className="absolute -bottom-1 -right-1 w-9 h-9 sm:w-12 sm:h-12 bg-slate-100 dark:bg-slate-700 rounded-xl sm:rounded-2xl overflow-hidden border-2 border-(--bg-main) shadow-md z-20 group-hover:translate-x-1 transition-transform duration-500">
                                <img src={getAvatarUrl(otherPartner.profileImageUrl, otherPartner.name)} alt="avatar" />

                            </div>
                        )}
                        {!otherPartner && match.type === 'Triple' && (
                            <div className="absolute -top-1 -right-1 p-1 bg-amber-400 text-white rounded-lg shadow-md border-2 border-(--bg-main) z-20">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                            <h4 className="text-lg font-bold text-(--text-main) truncate tracking-tight">
                                {mainPartner.name}
                                {otherPartner && <span className="text-(--text-muted) font-normal mx-1.5">&</span>}
                        ...
                        <div className="flex flex-wrap gap-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${isTriple
                                ...
                                {isTriple ? t('matches.type.chain') : t('matches.type.mutual')}
                            </span>
                            {hasActiveRequest && (
                ...
                <div className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-4 px-1 sm:px-4 mt-4 xl:mt-0 ${isTriple ? 'min-w-[260px]' : 'min-w-[180px]'}`}>
                    ...
                    <div className="flex-1 text-right group/loc min-w-0">
                        <p className={`text-[8px] sm:text-[10px] font-bold text-(--text-muted) uppercase tracking-widest mb-0.5 sm:mb-1 group-hover/loc:text-${accentColor}-500 transition-colors`}>{t('matches.labels.you')}</p>
                        <h5 className="text-xs sm:text-sm font-bold text-(--text-main) line-clamp-1 opacity-90">{sortedParticipants[0]?.stationName}</h5>
                        <p className="text-[8px] sm:text-[10px] font-medium text-(--text-muted) truncate">{sortedParticipants[0]?.stationDistrict}</p>
                    </div>

                    {/* Arrow 1 */}
                    <div className="relative flex flex-col items-center shrink-0 w-8 sm:w-12 group/arrow">
                        <div className="absolute inset-0 flex items-center justify-center">
                            <div className="w-full h-[2px] bg-(--border-main) relative overflow-hidden">
                    ...
                    <div className={`flex-1 ${isTriple ? 'text-center' : 'text-left'} group/loc min-w-0`}>
                        <p className={`text-[8px] sm:text-[10px] font-bold text-(--text-muted) uppercase tracking-widest mb-0.5 sm:mb-1 group-hover/loc:text-${accentColor}-500 transition-colors`}>{isTriple ? t('matches.labels.target') : t('matches.labels.partner')}</p>
                        <h5 className="text-xs sm:text-sm font-bold text-(--text-main) line-clamp-1 opacity-90">{sortedParticipants[1]?.stationName}</h5>
                        <p className="text-[8px] sm:text-[10px] font-medium text-(--text-muted) truncate">{sortedParticipants[1]?.stationDistrict}</p>
                    </div>
                    ...
                    {isTriple && (
                        <>
                            ...
                            <div className="flex-1 text-left group/loc min-w-0">
                                <p className="text-[8px] sm:text-[10px] font-bold text-(--text-muted) uppercase tracking-widest mb-0.5 sm:mb-1 group-hover/loc:text-indigo-500 transition-colors">{t('matches.labels.third')}</p>
                                <h5 className="text-xs sm:text-sm font-bold text-(--text-main) line-clamp-1 opacity-90">{sortedParticipants[2]?.stationName}</h5>
                                <p className="text-[8px] sm:text-[10px] font-medium text-(--text-muted) truncate">{sortedParticipants[2]?.stationDistrict}</p>
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between xl:justify-end xl:space-x-10 border-t xl:border-t-0 border-(--border-main) pt-6 xl:pt-0">
                    <div className="flex items-center space-x-6">
                        <div className="hidden sm:block text-right">
                            <p className="text-[10px] font-bold text-(--text-muted) uppercase tracking-widest mb-1">{t('matches.labels.relocation')}</p>
                            <div className={`flex items-center gap-1.5 text-${accentColor}-600 dark:text-${accentColor}-400`}>
                                ...
                                <span className="text-xs font-bold uppercase tracking-wider">{isTriple ? t('matches.type.chain_label') : t('matches.type.direct_label')}</span>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
                <div>
                    <h1 className="text-2xl sm:text-4xl font-semibold text-(--text-main) tracking-tight leading-tight">{t('matches.title')}</h1>
                    <p className="text-xs sm:text-sm text-(--text-muted) mt-2 font-medium">{t('matches.subtitle')}</p>
                </div>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
                <div className="col-span-1 bg-primary-600 rounded-2xl sm:rounded-[32px] p-4 sm:p-8 text-white flex flex-col sm:flex-row items-center sm:space-x-6 space-y-2 sm:space-y-0 shadow-2xl shadow-primary-900/10">
                    <div className="p-2 sm:p-4 bg-white/10 rounded-xl sm:rounded-[22px] backdrop-blur-md">
                        <Users size={20} className="sm:w-8 sm:h-8" />
                    </div>
                    <div className="text-center sm:text-left">
                        <p className="text-xl sm:text-2xl font-medium">{filteredMatches.length}</p>
                        <p className="text-[9px] sm:text-xs font-medium uppercase tracking-wider opacity-60">{t('matches.opportunities')}</p>
                    </div>
                </div>
                <div className="col-span-1 glass-card rounded-2xl sm:rounded-[32px] p-4 sm:p-8 flex flex-col sm:flex-row items-center sm:space-x-6 space-y-2 sm:space-y-0 border-(--border-main) bg-(--bg-card)">
                    <div className="p-2 sm:p-4 bg-slate-50 dark:bg-slate-800 rounded-xl sm:rounded-[22px]">
                        <CheckCircle2 size={20} className="sm:w-8 sm:h-8 text-emerald-500" />
                    </div>
                     <div className="text-center sm:text-left">
                        <p className="text-xl sm:text-2xl font-semibold text-(--text-main)">{filteredMatches.filter(m => m.type === 'Triple').length}</p>
                        <p className="text-[9px] sm:text-xs font-medium uppercase tracking-wider text-(--text-muted)">{t('matches.multi_way')}</p>
                    </div>
                </div>
                <div className="col-span-2 glass-card rounded-2xl sm:rounded-[32px] p-4 sm:p-8 border-(--border-main) bg-(--bg-card) flex flex-col sm:flex-row items-center justify-between shadow-sm relative overflow-hidden group/pref">
                    ...
                    <div className="grow relative z-10 text-center sm:text-left">
                        <div className="flex items-center justify-center sm:justify-start gap-2 mb-1.5">
                            ...
                            <h4 className="font-semibold text-(--text-main) tracking-tight text-sm sm:text-lg">{t('matches.preferences_title')}</h4>
                        </div>
                        <p className="text-[9px] sm:text-xs text-(--text-muted) font-medium hidden sm:block">{t('matches.preferences_desc')}</p>
                    </div>
                    <button onClick={() => navigate('/preferences')} className="relative z-10 text-primary-600 dark:text-primary-400 font-bold text-[9px] sm:text-xs uppercase tracking-wider hover:text-primary-800 dark:hover:text-primary-300 bg-primary-50 dark:bg-primary-900/20 hover:bg-primary-100 dark:hover:bg-primary-900/40 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl transition-colors cursor-pointer border border-primary-100 dark:border-primary-800 mt-2 sm:mt-0">{t('matches.adjust')}</button>
                </div>
            </div>            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-3 sm:px-4 bg-(--bg-card) p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-(--border-main)">
                    <div className="flex items-center space-x-3">
                        <h3 className="text-[10px] sm:text-sm font-bold text-(--text-main) uppercase tracking-wider">{t('matches.results')}</h3>
                        <span className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[9px] sm:text-[10px] font-bold rounded-lg uppercase tracking-tight">
                            {t('matches.found_count', { count: filteredMatches.length })}
                        </span>
                    </div>
                        ...
                        {['All', 'Direct', 'Triple'].map((type) => (
                            <button
                                ...
                                className={`relative px-4 sm:px-5 py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-[11px] font-bold transition-all duration-300 flex items-center space-x-2 whitespace-nowrap grow sm:grow-0 ${typeFilter === type
                                    ? 'bg-(--bg-card) text-primary-600 dark:text-primary-400 shadow-sm'
                                    : 'text-(--text-muted) hover:text-(--text-main)'
                                    }`}
                            >
                                <span>{t(`matches.filter.${type.toLowerCase()}`)}</span>
                ...
                {filteredMatches.length > 0 ? (
                    ...
                    <div className="p-12 glass-card rounded-[32px] text-center border-(--border-main) bg-(--bg-card)">
                        <MapPin size={48} className="mx-auto text-slate-200 dark:text-slate-800 mb-4" />
                        <p className="text-(--text-muted) font-medium">{t('matches.no_matches')}</p>
                        <button onClick={() => navigate('/preferences')} className="mt-4 text-primary-600 dark:text-primary-400 text-xs font-bold uppercase tracking-widest hover:text-primary-800 dark:hover:text-primary-300 transition-all cursor-pointer">{t('matches.set_preferences')}</button>
                    </div>
                )}
            ...
            {isMatchModalOpen && selectedMatch && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
                    ...
                    <div className="relative w-full max-w-2xl bg-(--bg-card) dark:backdrop-blur-2xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-(--border-main) animate-in zoom-in-95 duration-300">
 
                        <div className="p-6 sm:p-8 flex items-center justify-between border-b border-(--border-main) bg-(--bg-card) dark:bg-(--bg-card)/80 dark:backdrop-blur-xl z-20 sticky top-0">
                            <div>
                                <h2 className="text-2xl font-bold text-(--text-main) tracking-tight">{t('matches.modal.title')}</h2>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${selectedMatch.type === 'Triple' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-800' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800'}`}>
                                        {selectedMatch.type === 'Triple' ? t('matches.modal.type_triple') : t('matches.modal.type_direct')}
                                    </span>
                                </div>
                            </div>
                            <button
                                onClick={() => setIsMatchModalOpen(false)}
                                className="p-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 hover:text-(--text-main) rounded-full transition-all cursor-pointer"
                            >
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 sm:p-8 flex-1 overflow-y-auto custom-scrollbar relative z-10 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-xs font-bold text-(--text-muted) uppercase tracking-widest pl-3 border-l-2 border-primary-500">{t('matches.modal.proposed_cycle')}</h3>
                                </div>

                                <div className="bg-(--bg-main) dark:bg-(--bg-main)/50 rounded-[24px] border border-(--border-main) p-2 shadow-sm relative overflow-hidden">
                                    <div className="absolute left-10 top-10 bottom-10 w-0.5 bg-(--border-main) z-0 hidden sm:block"></div>
                                    {(() => {
                                                ...
                                                <div key={index} className="flex flex-col relative z-10 mb-2 last:mb-0 p-4">
                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                        <div className="flex items-center space-x-4">
                                                            <div className={`w-12 h-12 rounded-xl overflow-hidden shadow-sm shrink-0 border-2 ${isCurrentUser ? 'border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-900/20' : 'border-(--border-main) bg-(--bg-card)'}`}>
                                                                <img src={getAvatarUrl(displayImageUrl, displayName)} alt="avatar" />
                                                            </div>
                                                            <div>
                                                                <p className="font-bold text-(--text-main) text-base">
                                                                    {displayName} {isCurrentUser && <span className="text-[8px] ml-2 bg-primary-600 text-white px-2 py-0.5 rounded-full uppercase tracking-widest font-bold align-middle">{t('matches.labels.you')}</span>}
                                                                </p>
                                                                <p className="text-xs text-(--text-muted) font-medium flex items-center mt-1">
                                                        ...
                                                        <div className={`flex items-center space-x-3 p-3 rounded-2xl sm:w-auto w-full border ${isCurrentUser ? 'bg-primary-50 dark:bg-primary-900/10 border-primary-100 dark:border-primary-800' : 'bg-(--bg-card) border-(--border-main)/60'}`}>
                                                            <div className={`shrink-0 ${isCurrentUser ? 'text-primary-500' : 'text-slate-400'}`}>
                                                                <ArrowRightLeft size={16} />
                                                            </div>
                                                            <div>
                                                                <p className={`text-[9px] font-bold uppercase tracking-widest leading-none mb-1 ${isCurrentUser ? 'text-primary-400' : 'text-slate-500'}`}>{t('matches.labels.moves_to')}</p>
                                                                <p className="text-sm font-bold text-(--text-main) truncate max-w-[150px]">{nextPartner.stationName}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {index < sortedParticipants.length - 1 && (
                                                        <div className="absolute -bottom-3 left-1/2 sm:left-13 -translate-x-1/2 z-20">
                                                            <div className="bg-(--bg-card) text-slate-300 dark:text-slate-700 p-1 rounded-full border border-(--border-main) shadow-sm hidden sm:flex">
                                ...
                                <div className="relative z-10 flex-1">
                                    <p className="text-sm text-slate-400 dark:text-slate-400 font-medium mb-1">
                                        {(() => {
                                            ...
                                            if (target.requestStatus === 'PENDING') return t('matches.status.sent_msg', { station: target.stationName });
                                            if (target.requestStatus === 'ACCEPTED') return t('matches.status.established');
                                            if (target.requestStatus === 'PENDING_INCOMING') return t('matches.status.pending_incoming', { name: target.name });


                                            return t('matches.status.ready');
                                        })()}
                                    </p>
                                    <p className="font-semibold text-lg tracking-tight">
                                        {(() => {
                                            ...
                                            if (target.requestStatus === 'PENDING') return t('matches.status.awaiting');
                                            if (target.requestStatus === 'ACCEPTED') return t('matches.status.connected');
                                            if (target.requestStatus === 'PENDING_INCOMING') return t('matches.status.action_required');

                                            return t('matches.status.initiate');
                                        })()}
                                    </p>
                                </div>
                                ...
                                {(() => {
                                    ...
                                    if (target.requestStatus === 'PENDING') {
                                        return (
                                            <div className="flex items-center space-x-2 px-6 py-3.5 bg-white/10 rounded-xl text-slate-300 font-bold text-sm">
                                                <Clock size={16} />
                                                <span>{t('matches.actions.sending')}</span>
                                            </div>
                                        );
                                    }
                                    ...
                                    if (target.requestStatus === 'ACCEPTED') {
                                        return (
                                            ...
                                            >
                                                ...
                                                <span>{t('matches.actions.message')}</span>
                                            </button>
                                        );
                                    }
                                    ...
                                    if (target.requestStatus === 'PENDING_INCOMING') {
                                        return (
                                            ...
                                            >
                                                {t('matches.actions.view_request')}
                                            </button>
                                        );
                                    }
                                    ...
                                    return (
                                        ...
                                        >
                                            {isSending ? (
                                                ...
                                                <span>{t('matches.actions.send_request')}</span>
                                            )}
                                        </button>
                                    );
                                })()}
                            ...
                            <p className="text-[10px] text-(--text-muted) font-medium text-center px-6">
                                {t('matches.modal.note')}
                            </p>
                        </div>
                    </div>
                </div>
