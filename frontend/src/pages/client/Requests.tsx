...
const RequestCard = ({ request, type, onAction, onMessage, hasPackage }: any) => {
    ...
    const getStatusLabel = (status: string) => {
        switch (status) {
            case 'ACCEPTED': return t('requests.status_accepted');
            case 'REJECTED': return t('requests.status_rejected');
            default: return t('requests.status_pending');
        }
    };
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                    ...
                    {request.status === 'ACCEPTED' && (
                        ...
                        >
                            ...
                            <span>{t('requests.message_partner')}</span>
                        </button>
                    )}
                    ...
                    {request.status === 'PENDING' && !isIncoming && (
                        <div className="flex items-center space-x-2 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 rounded-xl text-xs font-bold italic">
                            <Clock size={14} />
                            <span>{t('requests.awaiting_response')}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            {request.status === 'ACCEPTED' && (
                <div className="mt-6 pt-6 border-t border-[var(--border-main)] grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    <div className="flex items-center space-x-3 p-2.5 sm:p-3 bg-[var(--bg-main)] dark:bg-[var(--bg-main)]/50 rounded-xl sm:rounded-2xl border border-[var(--border-main)]/50">
                        ...
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest leading-none mb-1">{t('requests.phone_label')}</p>
                    ...
                    <div className="flex items-center space-x-3 p-2.5 sm:p-3 bg-[var(--bg-main)] dark:bg-[var(--bg-main)]/50 rounded-xl sm:rounded-2xl border border-[var(--border-main)]/50">
                        ...
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest leading-none mb-1">{t('requests.email_label')}</p>
...
const Requests = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            toast.error(t('requests.toast_load_failed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAction = async (id: number, action: 'accept' | 'reject') => {
        try {
            ...
            const actionLabel = action === 'accept' ? t('requests.action_accept') : t('requests.action_reject');
            toast.success(t('requests.toast_action_success', { action: actionLabel }));
            fetchRequests();
        } catch (err) {
            console.error(`Failed to ${action} request`, err);
            const actionLabel = action === 'accept' ? t('requests.action_accept') : t('requests.action_reject');
            toast.error(t('requests.toast_action_failed', { action: actionLabel }));
        }
    };
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8 pb-10">
            <div>
                <h1 className="text-2xl sm:text-4xl font-semibold text-[var(--text-main)] tracking-tight leading-tight">{t('requests.title')}</h1>
                <p className="text-xs sm:text-sm text-[var(--text-muted)] mt-2 font-medium">{t('requests.subtitle')}</p>
            </div>
            ...
            <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-xl sm:rounded-2xl w-full sm:w-fit overflow-x-auto custom-scrollbar-hide">
                ...
                >
                    <Inbox size={16} />
                    <span>{t('requests.incoming_tab', { count: requests.incoming.length })}</span>
                </button>
                <button
                ...
                >
                    <Send size={16} />
                    <span>{t('requests.outgoing_tab', { count: requests.outgoing.length })}</span>
                </button>
            </div>
            ...
            <div className="space-y-4">
                {currentList.length > 0 ? (
                    ...
                    <div className="p-16 glass-card rounded-[32px] text-center border-[var(--border-main)] bg-[var(--bg-card)]">
                        ...
                        <p className="text-[var(--text-muted)] font-medium">{t('requests.no_requests', { type: activeTab === 'incoming' ? t('common.incoming') : t('common.outgoing') })}</p>
                        <button onClick={() => navigate('/matches')} className="mt-4 text-primary-600 dark:text-primary-400 text-xs font-bold uppercase tracking-widest hover:text-primary-800 dark:hover:text-primary-300 transition-all cursor-pointer">{t('requests.find_matches')}</button>
                    </div>
                )}
            </div>
        </div>
